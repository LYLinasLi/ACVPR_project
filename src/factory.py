...
def create_msls_dataloader(dataset, root_dir, cities, transform, batch_size):
    ...
    elif dataset == "triplet_soft_MSLS":
        ds = MSLSTripletDataSet(root_dir, cities, ds_key="fov", transform=transform)
    return DataLoader(ds, batch_size=batch_size, num_workers=4, shuffle=True)
...
def create_model(name, pool, last_layer=None, norm=None, p_gem=3, mode="siamese"):
    ...
    if mode == "siamese":
        return SiameseNet(backbone, pool, norm=norm, p=p_gem, name=name)
    elif mode == "triplet":
        return TripletNet(backbone, pool, norm=norm, p=p_gem, name=name)
    else:
        return BaseNet(backbone, pool, norm=norm, p=p_gem, name=name)