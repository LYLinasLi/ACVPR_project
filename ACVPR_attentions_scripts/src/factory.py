...
from attention.resnet import resnet_attention
from attention.cbam import ResNet50_CBAM
from attention.se import se_vgg16_bn, se_vgg16
from attention.gcnet2 import GC_resnet50
import timm
...
def get_backbone(name):
    if name == "resnet18":
    ...
    elif name == "pcam":    #triplet attention
        backbone = resnet_attention(depth=50)
        backbone = torch.nn.Sequential(*(list(backbone.children())[:-1]))
        output_dim = 2
    elif name == "gcnet":   
        backbone = GC_resnet50()
        ...
        # backbone = se_vgg16_bn(pretrained=True)
        backbone = se_vgg16(pretrained=True)
    elif name == "seresnt":
